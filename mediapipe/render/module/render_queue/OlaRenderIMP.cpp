...
//    imageData = nil;
    ...
    TextureInfo OLARenderIMP::render(TextureInfo inputTexture, bool exportFlag) {
        ...
        release();
#endif
        if (_lastWidth != inputTexture.width || _lastHeight != inputTexture.height) {
            release();
        }
        ...
        _loadProgram();
        Log("Opipe", "inputWidth:%d inputHeight:%d", inputTexture.width, inputTexture.height);
        ...
        Log("OlaRender", "aaa rs.len:%d", rs.len);
    ...
    void OLARenderIMP::setCanvasPixels(int width, int height, const void *pixels, int64_t frameTime,
        ...
        if (_blendTexture != -1) {
            if (_lastTransformSize != Vec2(width, height)) {
                Log("OlaRender", "setCanvasPixels _blendTexture !=");
                if (_transformTexture != - 1)
                {
                    Log("OlaRender", "setCanvasPixels _transformTexture: %d", _transformTexture);
                    glDeleteTextures(1, &_transformTexture);
                    _transformTexture = -1;
                }
            }
            ...
            if (_transformTexture == -1) {
                Log("OlaRender", "setCanvasPixels glGenTextures begin");
                glGenTextures(1, &_transformTexture);
                Log("OlaRender", "setCanvasPixels glGenTextures _transformTexture:%d", _transformTexture);
            }
            
            glBindTexture(GL_TEXTURE_2D, _transformTexture);
            Log("OlaRender", "setCanvasPixels glGenTextures glBindTexture end");
            Log("OlaRender", "setCanvasPixels glGenTextures glTexParameteri begin 1");
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
            Log("OlaRender", "setCanvasPixels glGenTextures glTexParameteri begin 2");
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
            Log("OlaRender", "setCanvasPixels glGenTextures glTexParameteri begin 3");
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
            Log("OlaRender", "setCanvasPixels glGenTextures glTexParameteri begin 4");
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
            Log("OlaRender", "setCanvasPixels glGenTextures glTexParameteri end");
            if (pixels != nullptr)
            {
                Log("OlaRender", "setCanvasPixels glGenTextures glTexImage2D before width:%d height:%d", width, height);
            } else {
                Log("OlaRender", "setCanvasPixels glGenTextures pixels null");
            }


            CHECK_GL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels));
            glBindTexture(GL_TEXTURE_2D, 0);
            Log("OlaRender", "setCanvasPixels glGenTextures glTexImage2D after");
            _lastTransformSize = Vec2(width, height);
            Log("OlaRender", "setCanvasPixels glGenTextures end");
            

            if (_transformTexture != -1 && _blendFbo != -1) {
                // test
                Log("OlaRender", "setCanvasPixels transform begin");
                _setROI(roi);
                ...
                CHECK_GL(glDrawArrays(GL_TRIANGLE_STRIP, 0, 4));
                Log("OlaRender", "setCanvasPixels transform end");
            }
        }
    }
