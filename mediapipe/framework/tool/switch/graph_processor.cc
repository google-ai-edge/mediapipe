...
absl::Status GraphProcessor::Initialize(CalculatorGraphConfig graph_config) {
  ...
  ABSL_ASSIGN_OR_RETURN(graph_input_map_,
                        tool::TagMap::Create(graph_config_.input_stream()));
  ABSL_ASSIGN_OR_RETURN(graph_output_map_,
...
absl::Status GraphProcessor::SendPacket(CollectionItemId id, Packet packet) {
  ABSL_RETURN_IF_ERROR(WaitUntilInitialized());
...
absl::Status GraphProcessor::ObserveGraph() {
  for (auto id = graph_output_map_->BeginId(); id != graph_output_map_->EndId();
    ...
    ABSL_RETURN_IF_ERROR(graph_->ObserveOutputStream(
        stream_name,
...
absl::Status GraphProcessor::Start() {
  ...
  ABSL_RETURN_IF_ERROR(graph_->Initialize(graph_config_, side_packets_));
  ABSL_RETURN_IF_ERROR(ObserveGraph());
  ABSL_RETURN_IF_ERROR(graph_->StartRun({}));
  return absl::OkStatus();
}
...
absl::Status GraphProcessor::Shutdown() {
  ...
  ABSL_RETURN_IF_ERROR(graph_->WaitUntilDone());
  graph_ = nullptr;
  return absl::OkStatus();
}
