...
// SpectrogramFloatCalculator provides a floating-point precision implementation
// of the MediaPipe spectrogram process.
//
// By using `float` instead of `double`, this calculator consumes less memory
// and can be more efficient on platforms where single-precision compute is
// prioritized.
class SpectrogramFloatCalculator : public CalculatorBase {
 public:
 ...
 private:
  ...
  std::vector<std::unique_ptr<audio_dsp::SpectrogramT<float>>>
      spectrogram_generators_;
...
REGISTER_CALCULATOR(SpectrogramFloatCalculator);
...
const float SpectrogramFloatCalculator::kLnSquaredMagnitudeToDb =
    4.342944819032518;
...
absl::Status SpectrogramFloatCalculator::Open(CalculatorContext* cc) {
  SpectrogramCalculatorOptions spectrogram_options =
      cc->Options<SpectrogramCalculatorOptions>();
  ...
  for (int i = 0; i < num_input_channels_; i++) {
    spectrogram_generators_.push_back(
        std::make_unique<audio_dsp::SpectrogramT<float>>());
...
absl::Status SpectrogramFloatCalculator::Process(CalculatorContext* cc) {
...
void SpectrogramFloatCalculator::Reset() {
...
template <class OutputMatrixType>
absl::Status SpectrogramFloatCalculator::ProcessVectorToOutput(
    const Matrix& input_stream,
    const OutputMatrixType postprocess_output_fn(const OutputMatrixType&),
    CalculatorContext* cc) {
...
absl::Status SpectrogramFloatCalculator::ProcessVector(
...
absl::Status SpectrogramFloatCalculator::Close(CalculatorContext* cc) {
