...
// Convert result OpenVINO tensors from classification models into MediaPipe
// classifications.
//
// Input:
//  TENSORS - Vector of ov::Tensor of type float32 containing one
...
// Usage example:
// node {
//   calculator: "OpenVINOTensorToClassificationCalculator"
...
//   }
// }
class OpenVINOTensorsToClassificationCalculator : public CalculatorBase {
 public:
 ...
 private:
  ::mediapipe::OpenVINOTensorsToClassificationCalculatorOptions options_;
  int top_k_ = 0;
  absl::node_hash_map<int, std::string> label_map_;
  bool label_map_loaded_ = false;
};
REGISTER_CALCULATOR(OpenVINOTensorsToClassificationCalculator);

absl::Status OpenVINOTensorsToClassificationCalculator::GetContract(
...
absl::Status OpenVINOTensorsToClassificationCalculator::Open(
    CalculatorContext* cc) {
  cc->SetOffset(TimestampDiff(0));

  options_ = cc->Options<
      ::mediapipe::OpenVINOTensorsToClassificationCalculatorOptions>();
...
absl::Status OpenVINOTensorsToClassificationCalculator::Process(
    CalculatorContext* cc) {
  ...
  const ov::Tensor& raw_score_tensor = input_tensors[0];
  RET_CHECK_EQ(raw_score_tensor.get_shape().size(), 2) << "Expecting tensor with 2 dimensions for classification results.";
  int num_classes = raw_score_tensor.get_shape()[1];
...
absl::Status OpenVINOTensorsToClassificationCalculator::Close(
    CalculatorContext* cc) {
  return absl::OkStatus();
}
