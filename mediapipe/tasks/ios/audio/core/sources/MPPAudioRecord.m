...
- (void)stop {
  ...
  // Using strong `self` (instance variable is available through strong self)
  // is okay since the block is shortlived and it'll release its strong reference
  // to `self` when it finishes execution.
  //
...
- (nullable MPPFloatBuffer *)readAtOffset:(NSUInteger)offset
  ...
  dispatch_sync(_convertLoadAndReadBufferQueue, ^{
    if (!_globalError) {
      bufferToReturn = [_floatRingBuffer floatBufferWithOffset:offset length:length error:error];
    } else {
      *error = [_globalError copy];
    }
  });
