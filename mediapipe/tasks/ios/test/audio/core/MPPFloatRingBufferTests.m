...
#define AssertEqualFloatBuffers(buffer, expectedBuffer)     \
  MPPFloatBuffer *outBuffer = ringBuffer.floatBuffer;       \
...
- (void)testLoadSucceedsWithPartialLengthBuffer {
  ...
  const float inputData[] = {1.0f, 2.0f, 3.0f};
...
- (void)testLoadSucceedsByShiftingOutOldElements {
  ...
  [self assertLoadSucceedsOnRingBuffer:ringBuffer
                         withInputData:&(firstInputData[0])
                       inputDataLength:firstInputDataLength
  ...
  [self
      assertLoadSucceedsOnRingBuffer:ringBuffer
                       withInputData:&(secondInputData[0])
                     inputDataLength:secondInputDataLength
...
- (void)testLoadSucceedsWithMostRecentElements {
  ...
  [self assertLoadSucceedsOnRingBuffer:ringBuffer
                         withInputData:&(firstInputData[0])
                       inputDataLength:ringBufferLength
      ...
      errorWithDomain:kExpectedErrorDomain
             ...
             userInfo:@{
               NSLocalizedDescriptionKey : [NSString
                   stringWithFormat:@"Index out of range. `offset` (%d) + `length` (%d) must be <= "
                                    @"`floatBuffer.length` (%d)",
...
- (void)testClearSucceeds {
  ...
  [self assertLoadSucceedsOnRingBuffer:ringBuffer
                         withInputData:&(firstInputData[0])
                       inputDataLength:firstInputDataLegth
                          expectedData:&(firstExpectedData[0])];
...
- (void)assertLoadSucceedsOnRingBuffer:(MPPFloatRingBuffer *)ringBuffer
                         withInputData:(float *)inputData
                       inputDataLength:(NSUInteger)inputDataLength
                          expectedData:(float *)expectedData {
  [self assertLoadSucceedsOnRingBuffer:ringBuffer
...
- (void)assertFloatBufferOfRingBuffer:(MPPFloatRingBuffer *)ringBuffer
                   equalsExpectedData:(float *)expectedData {
