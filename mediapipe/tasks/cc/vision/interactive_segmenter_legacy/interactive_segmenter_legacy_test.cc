...
TEST_F(CreateFromOptionsTest, FailsWithMissingModel) {
  absl::StatusOr<std::unique_ptr<InteractiveSegmenterLegacy>> segmenter =
      InteractiveSegmenterLegacy::Create(
          std::make_unique<InteractiveSegmenterLegacyOptions>());
...
TEST_F(CreateFromOptionsTest, FailsWithNeitherOutputSet) {
  ...
  EXPECT_THAT(InteractiveSegmenterLegacy::Create(std::move(options)),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("At least one of")));
}

struct InteractiveSegmenterLegacyTestParams {
...
class SucceedSegmentationWithRoi
    : public ::testing::TestWithParam<InteractiveSegmenterLegacyTestParams> {
 public:
  absl::StatusOr<RegionOfInterest> TestParamsToTaskOptions() {
    const InteractiveSegmenterLegacyTestParams& params = GetParam();
...
TEST_P(SucceedSegmentationWithRoi, SucceedsWithCategoryMask) {
  ...
  MP_ASSERT_OK_AND_ASSIGN(
      ...
      InteractiveSegmenterLegacy::Create(std::move(options)));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::ValuesIn<InteractiveSegmenterLegacyTestParams>(
        {// Keypoint input.
