...
TEST(GenaiUtilsTest, ChunkTextByCountingCharacters) {
  std::string input = "abcdefghijkl";
  auto result = ChunkTextByCountingCharacters(input, 10, 2);
  ASSERT_EQ(result.size(), 2);
  EXPECT_EQ(result[0], "abcdefghij");
  EXPECT_EQ(result[1], "ijkl");
}
...
struct GetTrailingPunctuationTestCase {
  std::string test_name;
  std::string original;
  std::string response;
  std::string expected_appended;
  std::string expected_response;
};

class GetTrailingPunctuationTest
    : public ::testing::TestWithParam<GetTrailingPunctuationTestCase> {};

TEST_P(GetTrailingPunctuationTest, ReturnsExpectedResult) {
  const GetTrailingPunctuationTestCase& test_case = GetParam();
  std::string response = test_case.response;
  std::string appended = GetTrailingPunctuation(test_case.original, response);
  response += appended;
  EXPECT_EQ(appended, test_case.expected_appended);
  EXPECT_EQ(response, test_case.expected_response);
}

INSTANTIATE_TEST_SUITE_P(
    GenaiUtilsTest, GetTrailingPunctuationTest,
    ::testing::ValuesIn<GetTrailingPunctuationTestCase>({
        {/*test_name=*/"AppendsPeriod", /*original=*/"This is a sentence.",
         /*response=*/"This is a sentence", /*expected_appended=*/".",
         /*expected_response=*/"This is a sentence."},
        {/*test_name=*/"DoesNotAppendIfAlreadyPresent",
         /*original=*/"Sentence.",
         /*response=*/"Sentence.", /*expected_appended=*/"",
         /*expected_response=*/"Sentence."},
        {/*test_name=*/"DoesNotAppendIfOriginalHasNoPunctuation",
         /*original=*/"Sentence", /*response=*/"Sentence",
         /*expected_appended=*/"",
         /*expected_response=*/"Sentence"},
    }),
    [](const ::testing::TestParamInfo<GetTrailingPunctuationTest::ParamType>&
           info) { return info.param.test_name; });
