...
} from '../../../../tasks/cc/vision/interactive_segmenter_legacy/proto/region_of_interest_pb';
...
import type {InteractiveSegmenterLegacyOptions} from './interactive_segmenter_legacy_options';
import {InteractiveSegmenterLegacyResult} from './interactive_segmenter_legacy_result';

export type {InteractiveSegmenterLegacyOptions} from './interactive_segmenter_legacy_options';
export * from './interactive_segmenter_legacy_result';
 ...
 * callback returns.
 */
export type InteractiveSegmenterLegacyCallback = (
  result: InteractiveSegmenterLegacyResult,
) => void;
 ...
 *   - batch is always 1
 */
export class InteractiveSegmenterLegacy extends VisionTaskRunner {
  ...
  private userCallback?: InteractiveSegmenterLegacyCallback;
  ...
  static createFromOptions(
    ...
    interactiveSegmenterLegacyOptions: InteractiveSegmenterLegacyOptions,
  ): Promise<InteractiveSegmenterLegacy> {
   ...
   */
  override setOptions(
    options: InteractiveSegmenterLegacyOptions,
  ): Promise<void> {
  ...
  segment(
    ...
    callback: InteractiveSegmenterLegacyCallback,
  ): void;
  /**
   * Performs interactive segmentation on the provided single image and invokes
   * the callback with the response. The method returns synchronously once the
  ...
  segment(
    ...
    imageProcessingOptionsOrCallback?:
      ...
      | InteractiveSegmenterLegacyCallback,
    callback?: InteractiveSegmenterLegacyCallback,
  ): InteractiveSegmenterLegacyResult | void {
  ...
  private processResults(): InteractiveSegmenterLegacyResult | void {
    try {
      const result = new InteractiveSegmenterLegacyResult(
