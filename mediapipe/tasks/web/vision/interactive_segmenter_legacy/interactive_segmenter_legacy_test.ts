...
import {RegionOfInterest as RegionOfInterestProto} from '../../../../tasks/cc/vision/interactive_segmenter_legacy/proto/region_of_interest_pb';
import {
...
import {
  ...
  RegionOfInterest,
} from './interactive_segmenter_legacy';
...
class InteractiveSegmenterLegacyFake
  extends InteractiveSegmenterLegacy
...
describe('InteractiveSegmenterLegacy', () => {
  let interactiveSegmenterLegacy: InteractiveSegmenterLegacyFake;

  beforeEach(async () => {
    addJasmineCustomFloatEqualityTester();
    interactiveSegmenterLegacy = new InteractiveSegmenterLegacyFake();
    await interactiveSegmenterLegacy.setOptions({
      baseOptions: {modelAssetBuffer: new Uint8Array([])},
    });
  });

  afterEach(() => {
    interactiveSegmenterLegacy.close();
  });

  it('initializes graph', async () => {
    verifyGraph(interactiveSegmenterLegacy);

    // Verify default options
    expect(interactiveSegmenterLegacy.categoryMaskListener).not.toBeDefined();
    expect(interactiveSegmenterLegacy.confidenceMasksListener).toBeDefined();
  });
  ...
  it('reloads graph when settings are changed', async () => {
    ...
    expect(interactiveSegmenterLegacy.categoryMaskListener).toBeDefined();
  });
  ...
  it("doesn't support region of interest", () => {
    expect(() => {
      interactiveSegmenterLegacy.segment(
        {} as HTMLImageElement,
        KEYPOINT,
  ...
  it('sends region-of-interest with keypoint', (done) => {
    interactiveSegmenterLegacy.fakeWasmModule._waitUntilIdle.and.callFake(
      () => {
        expect(interactiveSegmenterLegacy.lastRoi).toBeDefined();
        expect(
          interactiveSegmenterLegacy.lastRoi!.toObject().keypoint!,
        ).toEqual(
  ...
  it('sends region-of-interest with scribble', (done) => {
    interactiveSegmenterLegacy.fakeWasmModule._waitUntilIdle.and.callFake(
      () => {
        expect(interactiveSegmenterLegacy.lastRoi).toBeDefined();
        expect(
          interactiveSegmenterLegacy.lastRoi!.toObject().scribble!,
        ).toEqual(
          jasmine.objectContaining({
            pointList: [
  ...
  it('supports category mask', async () => {
    ...
    interactiveSegmenterLegacy.fakeWasmModule._waitUntilIdle.and.callFake(
      () => {
        expect(interactiveSegmenterLegacy.categoryMaskListener).toBeDefined();
        interactiveSegmenterLegacy.categoryMaskListener!(
    ...
    return new Promise<void>((resolve) => {
      interactiveSegmenterLegacy.segment(
        ...
        (result) => {
          expect(
            interactiveSegmenterLegacy.fakeWasmModule._waitUntilIdle,
          ).toHaveBeenCalled();
  ...
  it('supports confidence masks', async () => {
    ...
    interactiveSegmenterLegacy.fakeWasmModule._waitUntilIdle.and.callFake(
      () => {
        expect(
          interactiveSegmenterLegacy.confidenceMasksListener,
        ).toBeDefined();
        interactiveSegmenterLegacy.confidenceMasksListener!(
          [
  ...
  it('invokes listener after masks are available', async () => {
    ...
    interactiveSegmenterLegacy.fakeWasmModule._waitUntilIdle.and.callFake(
      () => {
        ...
        expect(listenerCalled).toBeFalse();
        interactiveSegmenterLegacy.qualityScoresListener!(qualityScores, 1337);
        expect(listenerCalled).toBeFalse();
      },
    );
  ...
  it('returns result', () => {
    ...
    const result = interactiveSegmenterLegacy.segment(
      {} as HTMLImageElement,
      KEYPOINT,
    );
    expect(result.confidenceMasks![0]).toBeInstanceOf(MPMask);
