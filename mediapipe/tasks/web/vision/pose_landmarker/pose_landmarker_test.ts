...
import type {PoseLandmarkerResult} from './pose_landmarker_result';
...
describe('PoseLandmarker', () => {
  ...
  it('transforms results', (done) => {
    ...
    const masks = [{data: new Float32Array([0, 1, 2, 3]), width: 2, height: 2}];
    ...
    poseLandmarker.detect({} as HTMLImageElement, (result) => {
      expect(
        poseLandmarker.getGraphRunner().addProtoToStream,
      ).toHaveBeenCalledTimes(1);
      expect(
        poseLandmarker.getGraphRunner().addGpuBufferAsImageToStream,
      ).toHaveBeenCalledTimes(1);
