...
constexpr float kImageDataPrecision = 1e-2;
...
void MatchesPoseLandmarkerResult(PoseLandmarkerResult* result,
  ...
  EXPECT_NEAR(result->pose_landmarks[0].landmarks[0].z, -0.1500f,
              landmark_precision);
  EXPECT_NEAR(result->pose_world_landmarks[0].landmarks[0].x, -0.0862f,
              landmark_precision);
...
TEST(PoseLandmarkerTest, ImageModeTest) {
  ...
  pose_landmarker_detect_image(landmarker, mp_image, &result,
                               /* error_msg */ nullptr);
  MatchesPoseLandmarkerResult(&result, kImageDataPrecision, kLandmarkPrecision);
...
TEST(PoseLandmarkerTest, VideoModeTest) {
  ...
  for (int i = 0; i < kIterations; ++i) {
    ...
    MatchesPoseLandmarkerResult(&result, kImageDataPrecision,
                                kLandmarkPrecision);
    pose_landmarker_close_result(&result);
  }
  pose_landmarker_close(landmarker, /* error_msg */ nullptr);
...
struct LiveStreamModeCallback {
  ...
  static void Fn(PoseLandmarkerResult* landmarker_result, const MpImage& image,
    ...
    MatchesPoseLandmarkerResult(landmarker_result, kImageDataPrecision,
                                kLandmarkPrecision);
