...
class MatrixTest(unittest.TestCase):
  ...
  def test_to_numpy_conversion(self):
    ...
    matrix_c = matrix_c_lib.MpMatrixC(rows=2, cols=3, data=c_data_array)
  ...
  def test_to_numpy_empty(self):
    matrix_c = matrix_c_lib.MpMatrixC(rows=0, cols=0, data=None)
    np_array = matrix_c.to_numpy()
    self.assertEqual(np_array.shape, (0,))

  def test_to_numpy_no_data(self):
    matrix_c = matrix_c_lib.MpMatrixC(rows=2, cols=3, data=None)
    np_array = matrix_c.to_numpy()
    self.assertEqual(np_array.shape, (0,))

  def test_to_numpy_data_is_copied(self):
    # Test that the C data is copied, since the C data is not owned by the
    # MpMatrixC instance but rather by the MediaPipe C API, which frees the data
    ...
    matrix_c = matrix_c_lib.MpMatrixC(rows=2, cols=2, data=c_data_array)
