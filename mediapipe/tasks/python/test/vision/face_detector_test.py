...
_PORTRAIT_ROTATED_EXPECTED_DETECTION = 'portrait_rotated_expected_detection.pbtxt'
_CAT_IMAGE = 'cat.jpg'
...
def _get_expected_face_detector_result(file_name: str) -> FaceDetectorResult:
  ...
  face_detection = detections_module.Detection.create_from_pb2(face_detection_proto)
...
class FaceDetectorTest(parameterized.TestCase):
  ...
  def _expect_keypoints_correct(self, actual_keypoints, expected_keypoints):
    ...
    for i in range(len(actual_keypoints)):
      self.assertAlmostEqual(
        actual_keypoints[i].x, expected_keypoints[i].x,
        delta=_KEYPOINT_ERROR_THRESHOLD)
      self.assertAlmostEqual(
        actual_keypoints[i].y, expected_keypoints[i].y,
        delta=_KEYPOINT_ERROR_THRESHOLD)

  def _expect_face_detector_results_correct(self, actual_results, expected_results):
    ...
    for i in range(len(actual_results.detections)):
      ...
      self._expect_keypoints_correct(actual_results.detections[i].keypoints,
  ...
  def test_detect(self, model_file_type, expected_detection_result_file):
    ...
    self._expect_face_detector_results_correct(detection_result,
  ...
  @parameterized.parameters(
    (ModelFileType.FILE_NAME, _PORTRAIT_EXPECTED_DETECTION),
    (ModelFileType.FILE_CONTENT, _PORTRAIT_EXPECTED_DETECTION))
  def test_detect_in_context(self, model_file_type, expected_detection_result_file):
    # Creates detector.
    if model_file_type is ModelFileType.FILE_NAME:
      base_options = _BaseOptions(model_asset_path=self.model_path)
    elif model_file_type is ModelFileType.FILE_CONTENT:
  ...
  @parameterized.parameters(
    (ModelFileType.FILE_NAME, _PORTRAIT_IMAGE, 0,
     _get_expected_face_detector_result(_PORTRAIT_EXPECTED_DETECTION)),
    (ModelFileType.FILE_CONTENT, _PORTRAIT_IMAGE, 0,
     _get_expected_face_detector_result(_PORTRAIT_EXPECTED_DETECTION)),
    (ModelFileType.FILE_NAME, _PORTRAIT_ROTATED_IMAGE, -90,
     _get_expected_face_detector_result(_PORTRAIT_ROTATED_EXPECTED_DETECTION)),
    (ModelFileType.FILE_CONTENT, _PORTRAIT_ROTATED_IMAGE, -90,
     _get_expected_face_detector_result(_PORTRAIT_ROTATED_EXPECTED_DETECTION)),
    (ModelFileType.FILE_NAME, _CAT_IMAGE, 0, FaceDetectorResult([])),
    (ModelFileType.FILE_CONTENT, _CAT_IMAGE, 0, FaceDetectorResult([])))
  def test_detect_for_video(self, model_file_type, test_image_file_name,
  ...
  @parameterized.parameters(
    ...
    (ModelFileType.FILE_NAME, _CAT_IMAGE, 0, FaceDetectorResult([])),
    (ModelFileType.FILE_CONTENT, _CAT_IMAGE, 0, FaceDetectorResult([])))
  def test_detect_async_calls(self, model_file_type, test_image_file_name,
                              rotation_degrees, expected_detection_result):
    # Creates detector.
    if model_file_type is ModelFileType.FILE_NAME:
      base_options = _BaseOptions(model_asset_path=self.model_path)
    ...
    def check_result(result: FaceDetectorResult, output_image: _Image,
                     timestamp_ms: int):
      self._expect_face_detector_results_correct(result,
                                                   expected_detection_result)
