...
from mediapipe.tasks.python.vision import interactive_segmenter_legacy
from mediapipe.tasks.python.vision.core import image as image_module
from mediapipe.tasks.python.vision.core import image_processing_options as image_processing_options_module

InteractiveSegmenterLegacyResult = (
    interactive_segmenter_legacy.InteractiveSegmenterLegacyResult
)
...
_InteractiveSegmenterLegacy = (
    interactive_segmenter_legacy.InteractiveSegmenterLegacy
)
_InteractiveSegmenterLegacyOptions = (
    interactive_segmenter_legacy.InteractiveSegmenterLegacyOptions
)
_RegionOfInterest = interactive_segmenter_legacy.RegionOfInterest
_Format = interactive_segmenter_legacy.RegionOfInterest.Format
...
class InteractiveSegmenterLegacyTest(parameterized.TestCase):
  ...
  def test_create_from_file_succeeds_with_valid_model_path(self):
    ...
    with _InteractiveSegmenterLegacy.create_from_model_path(
        self.model_path
    ) as segmenter:
      self.assertIsInstance(segmenter, _InteractiveSegmenterLegacy)

  def test_create_from_options_succeeds_with_valid_model_path(self):
    # Creates with options containing model file successfully.
    base_options = _BaseOptions(model_asset_path=self.model_path)
    options = _InteractiveSegmenterLegacyOptions(base_options=base_options)
    with _InteractiveSegmenterLegacy.create_from_options(options) as segmenter:
      self.assertIsInstance(segmenter, _InteractiveSegmenterLegacy)
  ...
  def test_create_from_options_fails_with_invalid_model_path(self):
    with self.assertRaisesRegex(
      ...
      options = _InteractiveSegmenterLegacyOptions(base_options=base_options)
      segmenter = _InteractiveSegmenterLegacy.create_from_options(options)
      segmenter.close()
  ...
  def test_segment_succeeds_with_category_mask(
    ...
    options = _InteractiveSegmenterLegacyOptions(
  ...
  def test_segment_fails_with_roi_in_image_processing_options(self):
    ...
    with self.assertRaises(ValueError):
      with _InteractiveSegmenterLegacy.create_from_options(
          options
      ) as segmenter:
        # Perform segmentation
