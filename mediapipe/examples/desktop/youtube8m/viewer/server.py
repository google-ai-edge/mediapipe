...
FLAGS = flags.FLAGS
flags.DEFINE_bool(
    "show_label_at_center", False, "Show labels at the center of the segment."
)
flags.DEFINE_integer("port", 8008, "Port that the API is served over.")
flags.DEFINE_string("tmp_dir", "/tmp/mediapipe", "Temporary asset storage location.")
...
class Youtube8MRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def fetch(self, path, segment_size):
        ...
        connection.request(
            "HEAD", filename_match.expand(r"/2/frame/train/%s" % filename)
        )
        response = connection.getresponse()
        ...
        curl_arg = "data.yt8m.org/2/j/i/%s/%s.js" % (contents[-5:-3], contents[-5:-1])
        print("Grab labels from", curl_arg)
        process = subprocess.Popen(
            ["curl", curl_arg], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
            ...
            for column in line.split(","):
                if first:
                    ...
                    if label_score:
                        ...
                        entry["labels"].append(
                            {"label": label_score.group(1), "score": score}
                        )
                    else:
                        print("empty score")
