...
SPLITS = {
    "train": """url,label index,label string,duration,credits
...
""",
    "test": """url,label index,label string,duration,credits
...
class DemoDataset(object):
    ...
    def as_dataset(
        ...
        def parse_fn(sequence_example):
            ...
            context_features = {
                ...
                ms.get_clip_label_index_key(): ms.get_clip_label_index_default_parser(),
                ms.get_clip_label_string_key(): ms.get_clip_label_string_default_parser(),
            }
            sequence_features = {
            ...
            classification_target = tf.one_hot(
                tf.sparse_tensor_to_dense(
                    parsed_context[ms.get_clip_label_index_key()]
                ),
                NUM_CLASSES,
            )
            images = tf.map_fn(
    ...
    def generate_examples(self, path_to_mediapipe_binary, path_to_graph_directory):
        ...
        for split in SPLITS:
            ...
            for row in reader:
                ...
                for start_time in range(0, int(row["duration"]), SECONDS_PER_EXAMPLE):
                    ...
                    ms.set_example_id(
                        bytes23(basename + "_" + str(start_time)), metadata
                    )
                    ms.set_clip_data_path(bytes23(local_path), metadata)
                    ms.set_clip_start_timestamp(
                        start_time * MICROSECONDS_PER_SECOND, metadata
                    )
                    ...
                    ms.set_clip_label_string((bytes23(row["label string"]),), metadata)
...
if __name__ == "__main__":
    flags.DEFINE_string("path_to_demo_data", "", "Path to directory to write data to.")
    flags.DEFINE_string(
        "path_to_mediapipe_binary",
        "",
    ...
    flags.DEFINE_string(
        "path_to_graph_directory", "", "Path to directory containing the graph files."
    )
    app.run(main)
