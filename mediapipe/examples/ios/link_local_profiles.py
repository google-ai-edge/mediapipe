...
example_names = {
    f for f in os.listdir(examples_ios) if os.path.isdir(os.path.join(examples_ios, f))
}


def configure_bundle_id_prefix(
    bundle_id_bzl=os.path.join(examples_ios, "bundle_id.bzl")
) -> str:
    ...
    """
    bundle_id_re = re.compile(r'^BUNDLE_ID_PREFIX\s*=\s*"(.*)"', flags=re.MULTILINE)
    ...
    if "*" in bundle_id_prefix:
        ...
        contents = (
            contents[: match.start(1)] + bundle_id_prefix + contents[match.end(1) :]
        )
...
def get_app_id(profile_path) -> Optional[str]:
    try:
        plist = subprocess.check_output(["security", "cms", "-D", "-i", profile_path])
...
def process_profile(profile_path, our_app_id_re):
    ...
    if app_dir_name not in example_names:
        print(
            f"The app id '{app_id}' has our prefix, but does not seem to match"
    ...
    link_path = os.path.join(
        examples_ios, app_dir_name, "provisioning_profile.mobileprovision"
    )
...
def main():
    ...
    our_app_id_re = re.compile(r"[0-9A-Z]+\." + re.escape(bundle_id_prefix) + r"\.(.*)")
