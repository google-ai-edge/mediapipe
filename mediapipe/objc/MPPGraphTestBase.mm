...
- (UIImage*)testImageNamed:(NSString*)name
    ...
    NSBundle* testBundle = [NSBundle bundleForClass:[self class]];
    NSURL* imageURL = subdirectory ? [testBundle URLForResource:name withExtension:extension subdirectory:subdirectory] : [testBundle URLForResource:name withExtension:extension];
...
#endif
    XCTAssert([self pixelBuffer:outputBuffer
                      isCloseTo:expectedBuffer
             maxLocalDifference:INT_MAX
           maxAverageDifference:1]);
