...
def function_with_default(f, default):
    ...
    return types.FunctionType(
        f.__code__, f.__globals__, f.__name__, (default,), f.__closure__
    )
...
def set_context_float(key, value, sequence, prefix=""):
    sequence.context.feature[merge_prefix(prefix, key)].float_list.value[:] = (value,)


def get_context_float(key, sequence, prefix=""):
    return sequence.context.feature[merge_prefix(prefix, key)].float_list.value[0]


def set_context_bytes(key, value, sequence, prefix=""):
    sequence.context.feature[merge_prefix(prefix, key)].bytes_list.value[:] = (value,)
...
def set_context_int(key, value, sequence, prefix=""):
    sequence.context.feature[merge_prefix(prefix, key)].int64_list.value[:] = (value,)
...
def get_feature_list_size(key, sequence, prefix=""):
    if has_feature_list(merge_prefix(prefix, key), sequence):
        return len(
            sequence.feature_lists.feature_list[merge_prefix(prefix, key)].feature
        )
    else:
        return 0
...
def get_float_list_at(key, index, sequence, prefix=""):
    return (
        sequence.feature_lists.feature_list[merge_prefix(prefix, key)]
        .feature[index]
        .float_list.value
    )
...
def add_float_list(key, values, sequence, prefix=""):
    ...
    ].feature.add().float_list.value[:] = values


def add_bytes_list(key, values, sequence, prefix=""):
    sequence.feature_lists.feature_list[
        merge_prefix(prefix, key)
    ].feature.add().bytes_list.value[:] = values


def add_int_list(key, values, sequence, prefix=""):
    sequence.feature_lists.feature_list[
        merge_prefix(prefix, key)
    ].feature.add().int64_list.value[:] = values
...
def add_float(key, value, sequence, prefix=""):
    sequence.feature_lists.feature_list[
        merge_prefix(prefix, key)
    ].feature.add().float_list.value[:] = (value,)


def add_bytes(key, value, sequence, prefix=""):
    sequence.feature_lists.feature_list[
        merge_prefix(prefix, key)
    ].feature.add().bytes_list.value[:] = (value,)


def add_int(key, value, sequence, prefix=""):
    sequence.feature_lists.feature_list[
        merge_prefix(prefix, key)
    ].feature.add().int64_list.value[:] = (value,)
