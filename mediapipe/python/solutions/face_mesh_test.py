...
class FaceMeshTest(parameterized.TestCase):
    def _annotate(
        self, frame: np.ndarray, results: NamedTuple, idx: int, draw_iris: bool
    ):
    ...
    def test_face(
        self, static_image_mode: bool, refine_landmarks: bool, num_frames: int
    ):
        image_path = os.path.join(os.path.dirname(__file__), "testdata/portrait.jpg")
        ...
        with mp_faces.FaceMesh(
            ...
            for idx in range(num_frames):
                ...
                for landmarks in results.multi_face_landmarks:
                    self.assertLen(
                        landmarks.landmark,
                        mp_faces.FACEMESH_NUM_LANDMARKS_WITH_IRISES
                        if refine_landmarks
                        else mp_faces.FACEMESH_NUM_LANDMARKS,
                    )
                ...
                for eye_idx, gt_lds in EYE_INDICES_TO_LANDMARKS.items():
                    prediction_error = np.abs(
                        np.asarray(multi_face_landmarks[0][eye_idx])
                        - np.asarray(gt_lds)
                    )
