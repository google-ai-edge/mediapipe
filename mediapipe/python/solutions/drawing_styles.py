...
from mediapipe.python.solutions import face_mesh_connections, hands_connections
...
_HAND_CONNECTION_STYLE = {
    hands_connections.HAND_PALM_CONNECTIONS: DrawingSpec(
        color=_GRAY, thickness=_THICKNESS_WRIST_MCP
    ),
    hands_connections.HAND_THUMB_CONNECTIONS: DrawingSpec(
        color=_PEACH, thickness=_THICKNESS_FINGER
    ),
    hands_connections.HAND_INDEX_FINGER_CONNECTIONS: DrawingSpec(
        color=_PURPLE, thickness=_THICKNESS_FINGER
    ),
    hands_connections.HAND_MIDDLE_FINGER_CONNECTIONS: DrawingSpec(
        color=_YELLOW, thickness=_THICKNESS_FINGER
    ),
    hands_connections.HAND_RING_FINGER_CONNECTIONS: DrawingSpec(
        color=_GREEN, thickness=_THICKNESS_FINGER
    ),
    hands_connections.HAND_PINKY_FINGER_CONNECTIONS: DrawingSpec(
...
_FACEMESH_CONTOURS_CONNECTION_STYLE = {
    ...
    face_mesh_connections.FACEMESH_LEFT_EYE: DrawingSpec(
        color=_GREEN, thickness=_THICKNESS_CONTOURS
    ),
...
def get_default_face_mesh_iris_connections_style() -> Mapping[
    Tuple[int, int], DrawingSpec
]:
...
def get_default_pose_landmarks_style() -> Mapping[int, DrawingSpec]:
    ...
    """
    pose_landmark_style = {}
    left_spec = DrawingSpec(color=(0, 138, 255), thickness=_THICKNESS_POSE_LANDMARKS)
    right_spec = DrawingSpec(color=(231, 217, 0), thickness=_THICKNESS_POSE_LANDMARKS)
