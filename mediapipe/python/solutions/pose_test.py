...
EXPECTED_POSE_LANDMARKS = np.array(
    [
        [460, 283],
        [467, 273],
        [471, 273],
...
class PoseTest(parameterized.TestCase):
    ...
    def _world_landmarks_list_to_array(self, landmark_list):
        return np.asarray([(lmk.x, lmk.y, lmk.z) for lmk in landmark_list.landmark])
    ...
    def _annotate_segmentation(self, segmentation, expected_segmentation, idx: int):
        ...
        self._segmentation_diff_to_rgb(expected_segmentation, segmentation).save(path)

    def _rgb_to_segmentation(
        self, img, back_color=(255, 0, 0), front_color=(0, 0, 255)
    ):
        img = np.array(img)
    ...
    def _segmentation_to_rgb(
        self, segm, back_color=(255, 0, 0), front_color=(0, 0, 255)
    ):
    ...
    def _segmentation_iou(self, segm_expected, segm_actual):
        ...
        result = intersection.sum() / (
            expected_dot.sum() + actual_dot.sum() - intersection.sum() + eps
        )
        return result

    def _segmentation_diff_to_rgb(
        self,
        segm_expected,
        segm_actual,
        ...
        with mp_pose.Pose(
            ...
            for idx in range(num_frames):
                ...
                self._assert_diff_less(
                    ...
                    EXPECTED_POSE_WORLD_LANDMARKS,
                    WORLD_DIFF_THRESHOLD,
                )
    ...
    @parameterized.named_parameters(("full", 1, "pose_squats.full.npz"))
    def test_on_video(self, model_complexity, expected_name):
        ...
        video_path = os.path.join(os.path.dirname(__file__), "testdata/pose_squats.mp4")
        expected_path = os.path.join(
            os.path.dirname(__file__), "testdata/{}".format(expected_name)
        )
