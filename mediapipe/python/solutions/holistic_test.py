...
EXPECTED_LEFT_HAND_LANDMARKS = np.array(
    [
        [958, 167],
        [950, 161],
        [945, 151],
...
EXPECTED_RIGHT_HAND_LANDMARKS = np.array(
    [
        [590, 135],
        [602, 125],
        [609, 114],
...
class PoseTest(parameterized.TestCase):
    def _landmarks_list_to_array(self, landmark_list, image_shape):
        ...
        return np.asarray(
            [(lmk.x * cols, lmk.y * rows) for lmk in landmark_list.landmark]
        )
    ...
    def test_on_image(
        self, static_image_mode, model_complexity, refine_face_landmarks, num_frames
    ):
        image_path = os.path.join(os.path.dirname(__file__), "testdata/holistic.jpg")
