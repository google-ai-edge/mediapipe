...
EXPECTED_HAND_COORDINATES_PREDICTION = [
    [
        [580, 34],
        [504, 50],
        [459, 94],
...
class HandsTest(parameterized.TestCase):
    ...
    def test_multi_hands(self, static_image_mode, model_complexity, num_frames):
        ...
        with mp_hands.Hands(
            ...
            for idx in range(num_frames):
                ...
                diff_threshold = (
                    LITE_MODEL_DIFF_THRESHOLD
                    if model_complexity == 0
                    else FULL_MODEL_DIFF_THRESHOLD
                )
    ...
    def _process_video(
        ...
        with mp_hands.Hands(
            ...
            while True:
                ...
                frame_landmarks = (
                    np.zeros([max_num_hands, num_landmarks, num_dimensions]) * np.nan
                )
                frame_w_landmarks = (
                    np.zeros([max_num_hands, num_landmarks, num_dimensions]) * np.nan
                )
    ...
    @parameterized.named_parameters(("full", 1, "asl_hand.full.npz"))
    def test_on_video(self, model_complexity, expected_name):
        ...
        video_path = os.path.join(
            os.path.dirname(__file__), "testdata/asl_hand.25fps.mp4"
        )
