...
class SolutionBaseTest(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
      ...
      """,
            ValueError,
            "must use matching tags and indexes.",
        ),
        (
      ...
      """,
            RuntimeError,
            'Unable to find the type for stream "in".',
        ),
    )
    ...
    """
        ...
        with solution_base.SolutionBase(graph_config=config_proto) as solution:
            ...
            self.assertEqual(
                results.output_detections.detection[0], expected_detection_1
            )
            self.assertEqual(
    ...
    @parameterized.named_parameters(
      ...
      node {
        ...
        output_stream: 'IMAGE:image_out'
            ...
            {"allow_signal": True, "rotation_degrees": 0},
        ),
    )
    ...
    def test_calculator_has_both_options_and_node_options(self):
        config_proto = text_format.Parse(
            CALCULATOR_OPTIONS_TEST_GRAPH_CONFIG, calculator_pb2.CalculatorGraphConfig()
        )
        with self.assertRaisesRegex(
            ValueError, "has both options and node_options fields."
        ):
            solution_base.SolutionBase(
                graph_config=config_proto,
                calculator_params={
    ...
    def _process_and_verify(
        self, config_proto, side_inputs=None, calculator_params=None
    ):
        input_image = np.arange(27, dtype=np.uint8).reshape(3, 3, 3)
        with solution_base.SolutionBase(
            graph_config=config_proto,
